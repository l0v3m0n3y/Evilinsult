...
class Evilinsult {
private:
    std::string api_base = "https://evilinsult.com";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("evilinsult.com"));
...
public:
    ...
    pplx::task<json::value> generate_insult(const std::string& lang) {
        return make_api_call("generate_insult.php?lang=" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(lang))) + "&type=json","GET");
    }
};
